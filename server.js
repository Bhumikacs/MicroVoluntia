...
// =====================
// Admin: Get All Pre-Registrations
// =====================
app.get("/admin/pre_registrations", async (req, res) => {
  try {
    const regs = await PreRegistration.find().sort({ createdAt: -1 });
    res.json(regs);
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
...
app.post("/submit", upload.single("taskFile"), async (req, res) => {
  try {
    ...
    const { fullName, taskName } = req.body;

    if (!user || !user.email) {
      return res.status(400).json({ message: "User info is missing" });
    }
    if (!fullName || !taskName) {
      return res.status(400).json({ message: "Full Name or Task Name is missing" });
    }
