...
// =====================
// Event Schema & Model (with description)
// =====================
const eventSchema = new mongoose.Schema({
  name: { type: String, required: true },
  date: { type: Date, required: true },
  location: { type: String, required: true },
  description: { type: String, required: true }, // description added
...
app.post("/submit", upload.single("taskFile"), async (req, res) => {
  try {
    const user = JSON.parse(req.body.user || "{}"); // user details sent from frontend
    if (!user || !user.email) return res.status(400).json({ message: "User not logged in" });

    const { fullName } = req.body;
    const taskFile = req.file;
    if (!taskFile) return res.status(400).json({ message: "Task PDF is required" });

    // Generate Certificate PDF
    const certName = `certificate-${Date.now()}.pdf`;
    const certPath = path.join(__dirname, "public/uploads", certName);
    ...
    doc.moveDown();
    doc.fontSize(24).text(fullName, { align: "center", underline: true });
    doc.moveDown();
    doc.fontSize(20).text(`has successfully completed their Micro-Task.`, { align: "center" });
    doc.moveDown(2);
    ...
    const newSubmission = new Submission({
      email: user.email,
      fullName,
      taskFile: `/uploads/${taskFile.filename}`,
      certificateFile: `/uploads/${certName}`
    });
    await newSubmission.save();

    res.status(201).json({ 
      message: "Task submitted successfully! Download your certificate below.", 
      certificateUrl: `/uploads/${certName}`
    });
...
// =====================
app.get("/admin/pre_registrations", async (req, res) => {
  try {
    const preRegs = await PreRegistration.find().sort({ createdAt: -1 });
    res.json(preRegs);
  } catch (err) {
